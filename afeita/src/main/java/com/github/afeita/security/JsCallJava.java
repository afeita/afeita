...
public class JsCallJava {
    ...
    public JsCallJava (String injectedName, Class injectedCls) {
        try {
            ...
            for (Method method : methods) {
                String sign;   //| Modifier.STATIC  去掉static ,非static 是可以
                if (method.getModifiers() != (Modifier.PUBLIC ) || (sign = genJavaMethodSign(method)) == null) {
                    continue;
                }
    ...
    public String call(Object obj,WebView webView, String jsonStr) {
        if (!TextUtils.isEmpty(jsonStr)) {
            try {
                ...
                return getReturn(jsonStr, 200, currMethod.invoke(obj, values));
            } catch (Exception e) {
                //优先返回详细的错误信息
                if (e.getCause() != null) {
